...
from models import Jogos, Times, Torcidas, Usuarios, Frases
...
def frase():
    ...
    jsoe = json.loads(response)

    resposta = jsoe["title"]

    data = date.today()

    novo_frase = Frases(Data = data,Frase = resposta)

    db.session.add(novo_frase)
    db.session.commit()

    return render_template('frases.html', resposta=resposta)


@app.route('/exportar/<titulo>')
def exportar(titulo):
    ...
    zip_file = zipfile.ZipFile('{0}.zip'.format(titulo), "w")
    ...
    return redirect(url_for(titulo.lower()))
